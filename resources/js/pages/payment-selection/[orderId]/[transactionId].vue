<template>
  <div class="vRPayment-payment-selection">
    <!-- Loading state -->
...
/** A line item of an order, as returned by vRPaymentGetOrderCheckoutData. */
interface OrderItem {
...
/** A VRPayment payment method available for the order. */
...
/** Response shape of vRPaymentGetOrderCheckoutData. */
...
/** Response shape of vRPaymentPayOrderRest. */
...
onMounted(async () => {
  ...
  if (paramsTransactionId) {
    ...
    sdk.plentysystems.vRPaymentGetTransactionFailure({ transactionId: paramsTransactionId })
      .then((result: any) => {
      ...
      .catch((err: unknown) => {
        console.warn('[vrpayment] Could not fetch transaction failure message:', err);
      });
  }

  try {
    const sdk = useSdk() as any;
    const result = await sdk.plentysystems.vRPaymentGetOrderCheckoutData({
    ...
    console.error('[vrpayment] Failed to load order checkout data:', err);
 ...
 * On success, redirects the browser to the VRPayment payment page.
 */
async function submitPayment(): Promise<void> {
  ...
  try {
    const sdk = useSdk() as any;
    const result = await sdk.plentysystems.vRPaymentPayOrderRest({
    ...
    if (responseData?.redirectUrl) {
      // Navigate to the VRPayment payment page. Keep `isSubmitting` true so the
    ...
    console.error('[vrpayment] Payment retry failed:', err);
 ...
 *
 *   :root {
 *     --vRPayment-color-primary: #ff6600;
 *     --vRPayment-font-family: 'Inter', sans-serif;
 *   }
 *
 * Color palette:
 *   --vRPayment-color-primary           accent color (selected state, buttons, radios, spinner)
 *   --vRPayment-color-primary-hover     submit button hover background
 *   --vRPayment-color-primary-disabled  submit button disabled background
 *   --vRPayment-color-primary-bg        background tint for the selected payment method
 *   --vRPayment-color-heading           primary heading text color
 *   --vRPayment-color-text              default body text color
 *   --vRPayment-color-text-secondary    secondary heading/text color
 *   --vRPayment-color-text-muted        muted/secondary text (e.g. quantities, descriptions)
 *   --vRPayment-color-text-faint        faint text (e.g. redirect notice)
 *   --vRPayment-color-text-light        light text (e.g. loading state)
 *   --vRPayment-color-border            default border color
 *   --vRPayment-color-border-strong     stronger border/divider color
 *   --vRPayment-color-border-hover      border color on hover (cancel button)
 *   --vRPayment-color-surface           surface background (order summary box)
 *   --vRPayment-color-error-bg          submission error background
 *   --vRPayment-color-error-border      submission error border
 *   --vRPayment-color-error-text        submission error text color
 *
 * Layout & typography:
 *   --vRPayment-font-family             base font family
 *   --vRPayment-max-width                max width of the component
 *   --vRPayment-radius                  default border radius (cards, buttons)
 *   --vRPayment-radius-sm               smaller border radius (error box)
 *   --vRPayment-payment-icon-width      payment method icon width
 *   --vRPayment-payment-icon-height     payment method icon height
 */
.vRPayment-payment-selection {
  max-width: var(--vRPayment-max-width, 600px);
  margin: 40px auto;
  padding: 0 20px;
  font-family: var(--vRPayment-font-family, system-ui, -apple-system, sans-serif);
}
...
.loading-container {
  ...
  height: 60vh;
  text-align: center;
  color: var(--vRPayment-color-text-light, #666);
}
...
.error-container h1 {
  font-size: 22px;
  color: var(--vRPayment-color-text-secondary, #333);
  margin-bottom: 8px;
}
.error-container p {
  color: var(--vRPayment-color-text-light, #666);
...
.redirect-notice {
  font-style: italic;
  font-size: 14px;
  color: var(--vRPayment-color-text-faint, #999);
  margin-top: 12px;
}
...
.payment-container h1 {
  font-size: 24px;
  color: var(--vRPayment-color-heading, #1a1a1a);
  margin-bottom: 8px;
}
.subtitle {
  color: var(--vRPayment-color-text, #555);
  margin-bottom: 28px;
  line-height: 1.5;
}
...
/* Order summary */
.order-summary {
  background: var(--vRPayment-color-surface, #f8f9fa);
  border: 1px solid var(--vRPayment-color-border, #e9ecef);
  border-radius: var(--vRPayment-radius, 8px);
  padding: 20px;
  margin-bottom: 28px;
}
.order-summary h2 {
...
.item-qty {
  white-space: nowrap;
  color: var(--vRPayment-color-text-muted, #888);
}
.order-total {
  display: flex;
  justify-content: space-between;
  border-top: 1px solid var(--vRPayment-color-border-strong, #dee2e6);
  margin-top: 12px;
  padding-top: 12px;
  font-size: 16px;
}
...
.payment-method-option {
  border: 2px solid var(--vRPayment-color-border, #e9ecef);
  border-radius: var(--vRPayment-radius, 8px);
  margin-bottom: 10px;
  transition: border-color 0.15s ease;
}
.payment-method-option.selected {
  border-color: var(--vRPayment-color-primary, #0d6efd);
  background: var(--vRPayment-color-primary-bg, #f0f6ff);
}
.payment-label {
  display: flex;
  align-items: center;
...
.payment-label input[type="radio"] {
  ...
  width: 18px;
  height: 18px;
  accent-color: var(--vRPayment-color-primary, #0d6efd);
}
.payment-icon {
  width: var(--vRPayment-payment-icon-width, 40px);
  height: var(--vRPayment-payment-icon-height, 28px);
  object-fit: contain;
  flex-shrink: 0;
}
.payment-info {
...
.submit-button {
  width: 100%;
  padding: 14px;
  background: var(--vRPayment-color-primary, #0d6efd);
  color: white;
  border: none;
  border-radius: var(--vRPayment-radius, 8px);
  font-size: 16px;
...
.submit-button:hover:not(:disabled) {
  background: var(--vRPayment-color-primary-hover, #0b5ed7);
}
.submit-button:disabled {
  background: var(--vRPayment-color-primary-disabled, #b0c4de);
  cursor: not-allowed;
}
...
.cancel-button {
  ...
  color: var(--vRPayment-color-text, #555);
  border: 2px solid var(--vRPayment-color-border-strong, #dee2e6);
  border-radius: var(--vRPayment-radius, 8px);
  font-size: 15px;
  font-weight: 500;
  cursor: pointer;
...
.cancel-button:hover {
  border-color: var(--vRPayment-color-border-hover, #adb5bd);
  color: var(--vRPayment-color-text-secondary, #333);
}
...
.submit-error {
  margin-top: 12px;
  padding: 12px 16px;
  background: var(--vRPayment-color-error-bg, #fff3f3);
  border: 1px solid var(--vRPayment-color-error-border, #f5c6cb);
  border-radius: var(--vRPayment-radius-sm, 6px);
  color: var(--vRPayment-color-error-text, #842029);
  font-size: 14px;
}
...
.spinner {
  ...
  height: 36px;
  border-radius: 50%;
  border-left-color: var(--vRPayment-color-primary, #0d6efd);
