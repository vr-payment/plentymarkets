...
export default defineNuxtPlugin((nuxtApp) => {
  ...
  function registerReturnContext() {
    vRPaymentRegisterReturnContext(window.location.origin, getActiveLang())
  ...
  function redirect(url: string) {
    if (!/^https?:\/\//i.test(url)) {
      console.error('[vrpayment] redirect value is not an absolute URL — the backend likely stored an error message as the redirect URL:', url);
    }
    ...
    XMLHttpRequest.prototype.send = function(body?: any) {
      ...
      if (url.toLowerCase().includes('doexecutepayment')) {
        xhr.addEventListener('readystatechange', function() {
          ...
          if (xhr.status !== 200) {
            console.error('[vrpayment] doExecutePayment returned non-200 status, no redirect will happen. status:', xhr.status, 'body:', xhr.responseText);
            return;
          }
          try {
            ...
            if ((data?.data?.type === 'redirect' || data?.data?.type === 'redirectUrl') && data?.data?.value) {
              ...
              console.warn('[vrpayment] doExecutePayment did not return a redirect — staying in shop. Full payload:', xhr.responseText);
            }
          } catch (err) {
            console.error('[vrpayment] Error parsing XHR response:', err, 'body:', xhr.responseText);
          }
        }, true);
      }
