...
function buildLineItem($orderItem, $uniqueId, $sku, $type, $basketNetPrices, $currencyDecimalPlaces, $itemAttributes, $prefixIfDuplicated)
{
    ...
    if ($basketNetPrices) {
        $lineItem->setAmountIncludingTax(VRPaymentSdkHelper::roundAmount($itemAmount['priceNet'] * $orderItem['quantity'], $currencyDecimalPlaces));
    } else {
        $lineItem->setAmountIncludingTax(VRPaymentSdkHelper::roundAmount($itemAmount['priceGross'] * $orderItem['quantity'], $currencyDecimalPlaces));
    }
...
function collectTransactionData($transactionRequest, $client)
{
    ...
    foreach ($order['orderItems'] as $key => $orderItem) {
        $prefixIfDuplicated = VRPaymentSdkHelper::checkForDuplicatePrefix($orderItem['itemVariationId'], $arrayOfItemIdsInLoop, $key);
