...
function collectTransactionData($transactionRequest, $client)
{
    ...
    foreach (SdkRestApi::getParam('basketItems') as $key => $basketItem) {
        $prefixIfDuplicated = VRPaymentSdkHelper::checkForDuplicatePrefix($basketItem['plenty_basket_row_item_variation_id'], $arrayOfItemIdsInLoop, $key);
        ...
        if ($basketNetPrices && isset($basketItem['vat']) && ! empty($basketItem['vat'])) {
            $lineItem->setAmountIncludingTax(VRPaymentSdkHelper::roundAmount(($basketItem['price'] / (1 + $basketItem['vat'] / 100)) * $basketItem['quantity'], $currencyDecimalPlaces));
        } else {
            $lineItem->setAmountIncludingTax(VRPaymentSdkHelper::roundAmount($basketItem['price'] * $basketItem['quantity'], $currencyDecimalPlaces));
        }
    ...
    if ($basket['shippingAmount'] > 0) {
        ...
        $lineItem->setAmountIncludingTax(VRPaymentSdkHelper::roundAmount($basket['shippingAmount'], $currencyDecimalPlaces));
    ...
    if ($basket['couponDiscount'] < 0) {
        ...
        $lineItem->setQuantity(1);
        $lineItem->setAmountIncludingTax(VRPaymentSdkHelper::roundAmount($basket['couponDiscount'], $currencyDecimalPlaces));
    ...
    if ($basket['paymentAmount'] > 0) {
        ...
        $lineItem->setQuantity(1);
        $lineItem->setAmountIncludingTax(VRPaymentSdkHelper::roundAmount($basket['paymentAmount'], $currencyDecimalPlaces));
    ...
    if (VRPaymentSdkHelper::roundAmount($lineItemTotalAmount, $currencyDecimalPlaces) > VRPaymentSdkHelper::roundAmount($basketAmount, $currencyDecimalPlaces)) {
        ...
        $lineItem->setQuantity(1);
        $lineItem->setAmountIncludingTax(VRPaymentSdkHelper::roundAmount($basketAmount - $lineItemTotalAmount, $currencyDecimalPlaces));
        $lineItem->setType('DISCOUNT');
        $lineItems[] = $lineItem;
    ...
    $filter->setChildren([
        ...
        VRPaymentSdkHelper::createEntityFilter('paymentMethod', $paymentMethodId)
    ]);
...
if (! empty($transactionId)) {
    ...
    $transactionRequest->setCustomersPresence(\VRPayment\Sdk\Model\CustomersPresence::VIRTUAL_PRESENT);
...
return VRPaymentSdkHelper::convertData($transactionResponse);
