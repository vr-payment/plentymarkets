...
class DirectDebitSepaPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('vRPayment.DirectDebitSepa_active') == "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.DirectDebitSepaTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
    ...
    {
        $fee = $this->configRepo->get('vRPayment.DirectDebitSepa_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.DirectDebitSepaDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('vRPayment::Payment.DirectDebitSepaIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
