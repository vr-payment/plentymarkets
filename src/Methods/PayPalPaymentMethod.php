...
class PayPalPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('vRPayment.PayPal_active') == "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.PayPalTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return 'PayPal';
    ...
    {
        $fee = $this->configRepo->get('vRPayment.PayPal_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.PayPalDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('vRPayment::Payment.PayPalIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
