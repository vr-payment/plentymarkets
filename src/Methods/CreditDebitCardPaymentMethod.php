...
class CreditDebitCardPaymentMethod extends AbstractPaymentMethod
{
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.CreditDebitCardTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
    ...
    {
        $fee = $this->configRepo->get('vRPayment.CreditDebitCard_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.CreditDebitCardDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('vRPayment::Payment.CreditDebitCardIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
