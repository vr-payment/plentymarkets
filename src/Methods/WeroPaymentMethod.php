...
class WeroPaymentMethod extends AbstractPaymentMethod
{
    use Loggable;
    ...
    {
        if ($this->configRepo->get('vRPayment.Wero_active') == "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.WeroTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return 'Wero';
    ...
    {
        $fee = $this->configRepo->get('vRPayment.Wero_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.WeroDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('vRPayment::Payment.WeroIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
