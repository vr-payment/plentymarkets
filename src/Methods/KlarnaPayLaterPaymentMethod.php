...
class KlarnaPayLaterPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('vRPayment.KlarnaPayLater_active') == "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.KlarnaPayLaterTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
    ...
    {
        $fee = $this->configRepo->get('vRPayment.KlarnaPayLater_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.KlarnaPayLaterDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('vRPayment::Payment.KlarnaPayLaterIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
