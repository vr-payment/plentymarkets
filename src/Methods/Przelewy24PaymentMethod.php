...
class Przelewy24PaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('vRPayment.Przelewy24_active') == "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.Przelewy24Title', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return 'Przelewy24';
    ...
    {
        $fee = $this->configRepo->get('vRPayment.Przelewy24_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.Przelewy24Description', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('vRPayment::Payment.Przelewy24IconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
