...
class IDealPaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('vRPayment.IDeal_active') == "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.IDealTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return 'iDeal';
    ...
    {
        $fee = $this->configRepo->get('vRPayment.IDeal_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.IDealDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('vRPayment::Payment.IDealIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
