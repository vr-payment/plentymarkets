...
class InvoicePaymentMethod extends AbstractPaymentMethod
{
    ...
    {
        if ($this->configRepo->get('vRPayment.Invoice_active') == "true") {
            return true;
        } else {
            return false;
        }
    ...
    public function getName(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.InvoiceTitle', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return 'Invoice';
    ...
    {
        $fee = $this->configRepo->get('vRPayment.Invoice_fee');
        if (! empty($fee)) {
            return (float) $fee;
        } else {
            return 0.00;
    ...
    public function getDescription(string $lang = 'de'): string
    {
        ...
        $title = $translator->trans('vRPayment::Payment.InvoiceDescription', [], $lang);
        if (! empty($title)) {
            return $title;
        } else {
            return '';
    ...
    public function getIcon(string $lang = 'de'): string
    {
        ...
        $iconUrl = $translator->trans('vRPayment::Payment.InvoiceIconUrl', [], $lang);
        if (!empty($iconUrl)) {
            return $iconUrl;
        } else {
