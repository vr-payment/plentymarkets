...
class PaymentService
{
    ...
    public function createWebhook()
    {
        ...
        $this->sdkService->call('createWebhook', [
            ...
            'notificationUrl' => $webstoreConfig->domainSsl . '/rest/v1/vrpayment/update-transaction' . ($this->config->get('plenty.system.info.urlTrailingSlash', 0) == 2 ? '/' : ''),
        ]);
    }
    ...
    {
        $this->getLogger(__METHOD__)->error('VRPayment::ExecutePaymentFunction', []);
        ...
        $this->getLogger(__METHOD__)->error('vRPayment::TransactionParameters', $parameters);
        ...
        $this->getLogger(__METHOD__)->error('VRPayment::ExecutePaymentBeforePwaRedirectReturn', [
            'type' => GetPaymentMethodContent::RETURN_TYPE_REDIRECT_URL,
    ...
    private function getSuccessUrl(?Order $order = null): string
    {
        ...
        $frontendOriginUrl = $frontendSession->getPlugin()->getValue('vRPaymentOriginUrl');
        $originUrl = $this->session->getPlugin()->getValue('vRPaymentOriginUrl');
        $this->getLogger(__METHOD__)->error('VRPayment::sessionGet', [
            'cookie' => $request->header('Cookie'),
            'sessionClass' => get_class($this->session),
            'frontendSessionClass' => get_class($frontendSession),
        ]);
        $this->getLogger(__METHOD__)->error('VRPayment::getSuccessUrl', [
            'originUrl' => $originUrl,
            'orderId' => $order->id,
        ...
        if ($originUrl && $order) {
            $this->getLogger(__METHOD__)->error('VRPayment::getSuccessUrlIsPWA', []);
            $accessKey = $this->orderRepository->generateAccessKey($order->id);
            $this->getLogger(__METHOD__)->error('VRPayment::accessKey', [
                'accessKey' => $accessKey
            ]);
    ...
    private function getFailedUrl(?Order $order = null): string
    {
        ...
        $this->getLogger(__METHOD__)->error('VRPayment::getFailedUrl', [
            'originUrl' => $originUrl,
            'order' => $order,
            'frontendOriginUrl' => $frontendOriginUrl,
        ]);
        if ($originUrl) {
            if ($order) {
                ...
                // VRPayment will automatically append /{transactionId} to this URL
                return sprintf(
            ...
            return sprintf('%s/checkout?vRPayment_failed=1', rtrim($originUrl, '/'));
        }
        $lang = $this->session->getLocaleSettings()->language;
        $domain = $this->webstoreHelper->getCurrentWebstoreConfiguration()->domainSsl;
    ...
    {
        $frontendSession = pluginApp(\Plenty\Modules\Frontend\Session\Storage\Contracts\FrontendSessionStorageFactoryContract::class);
        $originUrl = $frontendSession->getPlugin()->getValue('vRPaymentOriginUrl');
        if ($originUrl) {
