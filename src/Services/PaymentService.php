...
class PaymentService
{
     ...
     * 2. The vRPaymentOriginLang session value registered by the PWA via register-return.
    ...
    private function resolveLanguage(?Order $order = null): ?string
    {
        ...
        if ($order) {
            try {
                ...
                $this->getLogger(__METHOD__)->error('VRPayment::resolveLanguageOrderPropertyFailed', [
                    'orderId' => $order->id ?? null,
                    'error' => $e->getMessage(),
                ]);
            }
        }
        $sessionLang = $this->session->getPlugin()->getValue('vRPaymentOriginLang');
    ...
    private function getFailedUrl(?Order $order = null): string
    {
        ...
        if ($originUrl) {
            ...
            return sprintf('%s%s/checkout?vRPayment_failed=1', rtrim($originUrl, '/'), $langPrefix);
        }
        $lang = $this->resolveLanguage($order) ?: $this->session->getLocaleSettings()->language;
        $domain = $this->webstoreHelper->getCurrentWebstoreConfiguration()->domainSsl;
