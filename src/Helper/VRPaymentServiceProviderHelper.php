...
class VRPaymentServiceProviderHelper
{
    ...
    public function addAfterOrderCreatedListener(): void
    {
        $this->eventDispatcher->listen(OrderCreated::class, function (OrderCreated $event) {
            ...
            $this->getLogger(__METHOD__)->error('VRPayment::OrderCreatedEventFired', [
                'orderId' => $order->id,
            ]);
            ...
            try {
                ...
                if (!$this->isPwaContext()) {
                    $this->getLogger(__METHOD__)->error('VRPayment::OrderCreatedSkippedForCeres');
                    return;
                }
                ...
                if ($transactionId) {
                    try {
                        ...
                        $this->getLogger(__METHOD__)->error('VRPayment::TransactionLinked', [
                            'transactionId' => $transactionId,
                            'orderId' => $order->id,
                        ]);
                    } catch (\Exception $e) {
                    ...
                    $this->getLogger(__METHOD__)->error('VRPayment::NoTransactionInSession');
                }
                ...
                $this->getLogger(__METHOD__)->error('VRPayment::beforeExecutePaymentFunction', []);
                $result = $this->paymentService->executePayment($order, $paymentMethod);
                $this->getLogger(__METHOD__)->error('VRPayment::afterExecutePaymentFunction', [
                    'result' => $result,
                ]);
                ...
                $this->getLogger(__METHOD__)->error('VRPayment::OrderCreatedEventTypeMatch', [
                    'type' => $type,
                    '$result[content]' => $result['content']
                ]);
                // Store redirect URL in session so ExecutePayment listener can return it to PWA
                if ($type === 'redirect' && !empty($result['content'])) {
                    $this->getLogger(__METHOD__)->error('VRPayment::OrderCreatedEventSessionSet', [
                        'result[content]' => $result['content']
                    ]);
    ...
    public function addGetPaymentMethodContentEventListener(): void
    {
        $this->eventDispatcher->listen(GetPaymentMethodContent::class, function (GetPaymentMethodContent $event) {
            $this->getLogger(__METHOD__)->error('VRPayment::GetPaymentMethodContentEventFired', []);
            ...
            try {
                ...
                if (!$this->isPwaContext()) {
                    $this->getLogger(__METHOD__)->error('VRPayment::GetPaymentMethodContentSkippedForCeres');
                    return;
                }
                ...
                $this->getLogger(__METHOD__)->error('VRPayment::beforeExecutePaymentFromBasket', []);
    ...
    public function addExecutePaymentContentEventListener(): void
    {
        $this->eventDispatcher->listen(ExecutePayment::class, function (ExecutePayment $event) {
            ...
            $this->getLogger(__METHOD__)->debug('VRPayment::ExecutePaymentEventFired', [],);
            ...
            try {
                ...
                $this->getLogger(__METHOD__)->debug(
                    'VRPayment::ExecutePaymentContext',
                    [
                ...
                if ($isPwa) {
                    ...
                    $this->getLogger(__METHOD__)->debug(
                        'VRPayment::ExecutePaymentPwaRedirectUrl',
                        [
                    ...
                    if (empty($redirectUrl)) {
                        $transactionId = $this->session->getPlugin()->getValue('vRPaymentTransactionId');
                        $this->getLogger(__METHOD__)->error('VRPayment::ExecutePaymentPwaFallbackTransactionId', [
                            'transactionId' => $transactionId,
                        ]);
                        if ($transactionId) {
                            /** @var \VRPayment\Services\VRPaymentSdkService $sdkService */
                            ...
                            if (!empty($paymentPageUrl) && !is_array($paymentPageUrl)) {
                                ...
                                $this->getLogger(__METHOD__)->error('VRPayment::ExecutePaymentPwaFallbackUrlBuilt', [
                                    'url' => $redirectUrl,
                                ]);
                            }
                        }
                ...
                if (!$this->paymentHelper->isVRPaymentPaymentMopId($mopId)) {
                    $this->getLogger(__METHOD__)->error('VRPayment::ExecutePaymentNotVRPaymentMethod', [
                        'mop' => $mopId,
                    ]);
                    return;
                }
                ...
                if (!$eventMop) {
                    $this->getLogger(__METHOD__)->error('VRPayment::ExecutePaymentMethodNull', ['mop' => $mopId]);
                    return;
                }

                $eventOrderId = $this->orderRepository->findById($orderId);
                if (!$eventOrderId) {
                    $this->getLogger(__METHOD__)->error('VRPayment::ExecutePaymentOrderNotFound', [
                        'orderId' => $orderId,
                    ]);
                    return;
                }
                ...
                $this->getLogger(__METHOD__)->error('VRPayment::ExecutePaymentCeresResult', ['result' => $result]);
