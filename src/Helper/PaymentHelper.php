...
class PaymentHelper
{
    ...
    /**
     * Returns the VR Payment payment method's id.
     *
        ...
        if (! is_null($paymentMethods)) {
            foreach ($paymentMethods as $paymentMethod) {
                ...
                if ($paymentMethod->id == $mopId) {
                    $this->getLogger(__METHOD__)->error('VRPayment::isVRPaymentPaymentMopId_TRUE', [
                        'mopId' => $mopId,
    ...
    public function updatePlentyPayment(array $transaction): bool
    {
        ...
        $this->getLogger(__METHOD__)->error(
            'VRPayment::updatePlentyPaymentWebhook',
            [
        ...
        if (empty($payments)) {
            $this->getLogger(__METHOD__)->error(
                'VRPayment::NoMatchingPaymentForWebhook',
                [
                    'transactionId' => $transaction['id'],
                    'state' => $transaction['state'],
    ...
    public function updateInvoice($transactionInvoice)
    {
        ...
        $this->getLogger(__METHOD__)->error(
            'VRPayment::updateInvoiceWebhook',
            [
