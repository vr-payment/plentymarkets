...
class PaymentProcessController extends Controller
{
    ...
    {
        $this->getLogger(__METHOD__)->error('VRPayment::ReturnFailedHit', [
        ...
        if (is_array($transaction) && isset($transaction['error'])) {
            $this->getLogger(__METHOD__)->error('VRPayment::ReturnFailedLookupFailed', [
                'transactionId' => $transactionId,
                'transaction' => $transaction,
            ]);
        ...
        // $this->getLogger(__METHOD__)->error('VRPayment::ReturnFailedState', [
        //     'transactionId' => $transactionId,
        //     'state' => $state,
        // ]);
        ...
        $this->getLogger(__METHOD__)->error('VRPayment::ReturnFailedPayments', [
            'payments' => $payments,
            'payment' => $payment,
        ]);
        ...
        if ($payment) {
            $this->getLogger(__METHOD__)->error('VRPayment::ReturnFailedPaymentIsPresent', []);
            $orderRelation = $this->paymentOrderRelationRepository->findOrderRelation($payment);
            $this->getLogger(__METHOD__)->error('VRPayment::ReturnFailedPaymentOrderRelation', [
        ...
        $this->getLogger(__METHOD__)->error('VRPayment::ReturnFailedOrderAfterFirstLookup', [
            'order' => $order
        ]);

        // Keep the order in its current unpaid state so it can be reused for payment retry
        if ($order) {
            $this->getLogger(__METHOD__)->error('VRPayment::OrderKeptForRetry', [
                'orderId' => $order->id,
        ...
        $this->getLogger(__METHOD__)->error('VRPayment::OrderCanceledAfterPlentyPaymentUpdate', [
           'transaction' => $transaction,
        ]);

        if (!empty($transaction['userFailureMessage'])) {
            $this->getLogger(__METHOD__)->error('VRPayment::OrderCanceledSetFailReason', [
                'userFailureMessage' => $transaction['userFailureMessage'],
            ]);
            $this->frontendSession->getPlugin()->setValue(
                'vRPaymentPayErrorMessage',
    ...
    {
        $this->getLogger(__METHOD__)->error('VRPayment::RedirectToCheckoutPWAHit', [
            'orderId' => $orderId,
        ]);
        $originUrl = $this->frontendSession->getPlugin()->getValue('vRPaymentOriginUrl');
        $this->getLogger(__METHOD__)->error('VRPayment::RedirectToCheckoutPWAOriginUrlCheck', [
            'originUrl' => $originUrl,
            'orderId' => $orderId,
        ]);
        if ($originUrl) {
            if ($orderId) {
                ...
                $url = sprintf('%s/checkout?vRPayment_failed=1', rtrim($originUrl, '/'));
            }
            $this->getLogger(__METHOD__)->error('VRPayment::RedirectToCheckoutRedirectToPwa', [
                'url' => $url,
            ]);
        ...
        $this->getLogger(__METHOD__)->error('VRPayment::RedirectToCheckoutRedirectToOG', [
            'url' => $url,
        ]);
        return $this->response->redirectTo($url);
    }
    ...
    public function preparePayment(Request $request)
    {
        ...
        try {
            ...
            // Check if this is a VRPayment method
            ...
            $this->getLogger(__METHOD__)->info('PWA preparing payment from basket', [
                'sessionVRPaymentOriginUrl' => $this->frontendSession->getPlugin()->getValue('vRPaymentOriginUrl') ?? 'null',
                'sessionVRPaymentTransactionId' => $this->frontendSession->getPlugin()->getValue('vRPaymentTransactionId') ?? 'null',
            ]);
...
//            ]);
    ...
    {
        $this->getLogger(__METHOD__)->error('VRPayment::sessionSet', [
            'cookie' => $request->header('Cookie'),
            'sessionClass' => get_class($this->frontendSession),
        ]);
        $this->getLogger(__METHOD__)->error('VRPayment::registerReturnContextHit', [
            'request' => $request
        ]);
        $originUrl = $request->input('originUrl');
        $lang = $request->input('lang') ?: 'en';

        $this->getLogger(__METHOD__)->error('VRPayment::registerReturnContextVars', [
            'originUrl' => $originUrl,
            'lang' => $lang,
        ]);

        if ($originUrl) {
            $this->getLogger(__METHOD__)->error('VRPayment::registerReturnContextSetToSession', []);
            $this->frontendSession->getPlugin()->setValue('vRPaymentOriginUrl', $originUrl);
            $this->frontendSession->getPlugin()->setValue('vRPaymentOriginLang', $lang);
    ...
    public function restoreCart(Request $request)
    {
        ...
        $this->getLogger(__METHOD__)->error('VRPayment::restoreCartOrderId', [
            'orderId' => $orderId,
        ]);
        if (!$orderId) {
        ...
        $this->getLogger(__METHOD__)->error('VRPayment::restoreCartOrder', [
            'order' => $order,
        ]);
        if (!$order) {
        ...
        // $paymentMethodId = $this->orderHelper->getOrderPropertyValue($order, OrderPropertyType::PAYMENT_METHOD);
        // if (!$paymentMethodId || !$this->paymentHelper->isVRPaymentPaymentMopId($paymentMethodId)) {
        $isVRPaymentPayment = $this->paymentHelper->isVRPaymentPaymentMopId($order->methodOfPaymentId);
        $this->getLogger(__METHOD__)->error('VRPayment::restoreCartIsVRPaymentPayment', [
            'isVRPaymentPayment' => $isVRPaymentPayment,
        ]);
        if (!$isVRPaymentPayment) {
            return $this->response->json(['ok' => false, 'reason' => 'order does not belong to vrpayment payment method'], 403);
        }
        ...
        $this->getLogger(__METHOD__)->error('VRPayment::restoreCartFinish', []);
    ...
    public function getOrderCheckoutData(Request $request)
    {
        try {
            ...
            // Verify the order belongs to a VRPayment payment method
            if (!$this->paymentHelper->isVRPaymentPaymentMopId($order->methodOfPaymentId)) {
                return $this->response->make(
                    json_encode(['error' => 'Order does not use a VRPayment payment method']),
                    403,
                    ['Content-Type' => 'application/json'],
                );
            }
            ...
            $this->getLogger(__METHOD__)->error('VRPayment::GetOrderCheckoutData', [
                'orderId' => $orderId,
            ...
            $this->getLogger(__METHOD__)->error('VRPayment::GetOrderCheckoutDataException', [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
     ...
     * Switches payment method and creates a new VRPayment transaction for the same order
     *
     * @param Request $request
     * @return Response
     */
    public function payOrderRest(Request $request)
    {
        try {
            ...
            // Execute payment using the updated order, creating a new VRPayment transaction
            $paymentMethod = $this->paymentMethodService->findByPaymentMethodId($paymentMethodId);
            $result = $this->paymentService->executePayment($order, $paymentMethod);

            $this->getLogger(__METHOD__)->error('VRPayment::PayOrderRestResult', [
                'orderId' => $orderId,
                'paymentMethodId' => $paymentMethodId,
            ...
            // Payment processing error from VRPayment
            ...
            $this->getLogger(__METHOD__)->error('VRPayment::PayOrderRestException', [
                'message' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
